...
ApkTools="java -jar -Xms2048m -Xmx2048m $SHELL_PATH/apktool.jar"
AndroidApi=$(getProp 'ro.build.version.sdk')

SmallTempPath=../../OUT/SmallTemp
...
doUnApk(){
    ...
    $SHELL_PATH/zipalign 4 "$APK_OUT_FILE" "${APK_OUT_FILE}.zipalign"
...
SmallPatchPath=./SmallPatchFile

for line in $(cat ${SmallPatchPath}/FileConfig.ini) ; do
